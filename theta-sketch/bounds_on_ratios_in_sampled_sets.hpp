...
private:
  ...
  static void check_inputs(uint64_t a, uint64_t b, double f) {
    if (a < b) {
      std::cerr << "Invalid argument: a must be >= b: a = " + std::to_string(a) + ", b = " + std::to_string(b) << std::endl;
      exit(1);
    }
    if ((f > 1.0) || (f <= 0.0)) {
      std::cerr << "Invalid argument: Required: ((f <= 1.0) && (f > 0.0)): " + std::to_string(f) << std::endl;
      exit(1);
    }
  }
