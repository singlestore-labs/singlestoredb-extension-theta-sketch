...
template<typename A>
void update_theta_sketch_alloc<A>::update_by_hash(const uint64_t _hash) {
...
template<typename A>
bool compact_theta_sketch_alloc<A>::serialize2(unsigned char** outBytes, size_t* outSize) const {
  unsigned header_size_bytes = 0;
  const uint8_t preamble_longs = this->is_estimation_mode() ? 3 : this->is_empty() || entries_.size() == 1 ? 1 : 2;
  const size_t size = header_size_bytes + sizeof(uint64_t) * preamble_longs
      + sizeof(uint64_t) * entries_.size();
  *outBytes = (unsigned char*) malloc(size);
  if (!*outBytes)
  {
      return false;
  }
  *outSize = size;
  uint8_t* ptr = *outBytes + header_size_bytes;
...
compact_theta_sketch_alloc<A> compact_theta_sketch_alloc<A>::deserialize(std::istream& is, uint64_t seed, const A& allocator) {
  ...
  switch (serial_version) {
    ...
    default:
      THROW_INVALID_ARG("unexpected sketch serialization version " + std::to_string(serial_version));
  }
}
    ...
    uint8_t, std::istream& is, uint64_t seed, const A& allocator)
{
  ...
  if (!is.good()) THROW_RUNTIME_ERR("error reading from std::istream");
    ...
    uint8_t preamble_longs, std::istream& is, uint64_t seed, const A& allocator)
{
  ...
  if (preamble_longs == 1) {
    ...
    THROW_INVALID_ARG(std::to_string(preamble_longs) + " longs of premable, but expected 1, 2, or 3");
  }
}
