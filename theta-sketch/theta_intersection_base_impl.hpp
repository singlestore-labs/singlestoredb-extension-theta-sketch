...
void theta_intersection_base<EN, EK, P, S, CS, A>::update(SS&& sketch) {
  ...
  if (!sketch.is_empty() && sketch.get_seed_hash() != compute_seed_hash(table_.seed_)) THROW_INVALID_ARG("seed hash mismatch")
  ...
  if (!is_valid_) { // first update, copy or move incoming sketch
    ...
    for (auto&& entry: sketch) {
      ...
      if (result.second) {
        THROW_INVALID_ARG("duplicate key, possibly corrupted input sketch");
      }
      table_.insert(result.first, conditional_forward<SS>(entry));
    }
    if (table_.num_entries_ != sketch.get_num_retained()) THROW_INVALID_ARG("num entries mismatch, possibly corrupted input sketch");
  } else { // intersection
    ...
    for (auto&& entry: sketch) {
      if (EK()(entry) < table_.theta_) {
        auto result = table_.find(EK()(entry));
        if (result.second) {
          if (match_count == max_matches) THROW_INVALID_ARG("max matches exceeded, possibly corrupted input sketch");
    ...
    if (count > sketch.get_num_retained()) {
      THROW_INVALID_ARG(" more keys than expected, possibly corrupted input sketch");
    } else if (!sketch.is_ordered() && count < sketch.get_num_retained()) {
      THROW_INVALID_ARG(" fewer keys than expected, possibly corrupted input sketch");
    }
    if (match_count == 0) {
      table_ = hash_table(0, 0, resize_factor::X1, 1, table_.theta_, table_.seed_, table_.allocator_, table_.is_empty_);
...
CS theta_intersection_base<EN, EK, P, S, CS, A>::get_result(bool ordered) const {
  if (!is_valid_) THROW_INVALID_ARG("calling get_result() before calling update() is undefined");
