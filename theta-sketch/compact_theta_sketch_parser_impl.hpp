...
auto compact_theta_sketch_parser<dummy>::parse(const void* ptr, size_t size, uint64_t seed, bool dump_on_error) -> compact_theta_sketch_data {
  if (size < 8) THROW_OUT_OF_RANGE("at least 8 bytes expected, actual " + std::to_string(size)
  ...
  case COMPACT_SKETCH_SERIAL_VERSION: {
      ...
      if (has_theta) {
        if (size < 16) THROW_OUT_OF_RANGE("at least 16 bytes expected, actual " + std::to_string(size));
      ...
      if (size < expected_size_bytes) {
        THROW_OUT_OF_RANGE(std::to_string(expected_size_bytes) + " bytes expected, actual " + std::to_string(size)
            + (dump_on_error ? (", sketch dump: " + hex_dump(reinterpret_cast<const uint8_t*>(ptr), size)) : ""));
      }
  ...
  case 2:  {
      ...
      if (preamble_size == 1) {
          ...
          THROW_INVALID_ARG(std::to_string(preamble_size) + " longs of premable, but expected 1, 2, or 3");
      }
  }
  default:
      ...
      THROW_INVALID_ARG("unexpected sketch serialization version");
  }
}
