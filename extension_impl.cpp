...
template<typename F>
extension_state_t join_sketches(extension_state_t left, extension_state_t right, F&& f) {
...
extension_state_t extension_sketch_init() {
    auto* sketch = new update_theta_sketch(update_theta_sketch::builder().build());
    extension_state_t handle = reinterpret_cast<extension_state_t>(sketch);
    DEBUG_LOG("[INIT] handle=%d\n", handle);
    return handle;
}

extension_state_t extension_sketch_update(extension_state_t handle, int32_t input) {
...
void extension_sketch_serialize(extension_state_t handle, extension_list_u8_t *data) {
