...
private:
  ...
  static inline void check_split_points(const T* items, uint32_t size) {
    for (uint32_t i = 0; i < size ; i++) {
      if (std::isnan(items[i])) {
        THROW_INVALID_ARG("Values must not be NaN");
      }
      if ((i < (size - 1)) && !(Comparator()(items[i], items[i + 1]))) {
        THROW_INVALID_ARG("Values must be unique and monotonically increasing");
      }
    }
  }
  ...
  static inline void check_split_points(const T* items, uint32_t size) {
    for (uint32_t i = 0; i < size ; i++) {
      if ((i < (size - 1)) && !(Comparator()(items[i], items[i + 1]))) {
        THROW_INVALID_ARG("Items must be unique and monotonically increasing");
      }
    }
  }
};
